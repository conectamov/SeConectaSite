...
function onDocClick(e) {
  if (!e.target.closest('[data-menu]') && !e.target.closest('[data-social]')) closeAll()
}
...
<template>
  <header v-if="$route.path !== '/login' && $route.path !== '/signup'" class="w-full z-50 top-0">
      ...
      <nav class="hidden lg:flex items-center gap-1">
        ...
        <div class="relative" data-menu @mouseenter="hoverProgramas" @mouseleave="leaveProgramas">
          <button class="flex items-center gap-1 text-[0.88rem] text-gray-600 font-medium hover:text-[#079272] transition-colors px-3 py-2 rounded-lg hover:bg-[#f7f5f0] border-none bg-transparent cursor-pointer" :class="showProgramas && 'bg-[#f7f5f0] text-[#079272]'" @click.stop="toggleProgramas">
            Programas
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" :class="['transition-transform duration-200', showProgramas ? 'rotate-180' : '']">
      ...
      <div class="flex items-center gap-1 md:gap-2">
          ...
          <Transition enter-from-class="opacity-0 translate-y-1 scale-95" enter-active-class="transition-all duration-150 origin-top-right" leave-to-class="opacity-0 translate-y-1 scale-95" leave-active-class="transition-all duration-100 origin-top-right">
            <div v-if="showNotifs" class="absolute right-0 top-[calc(100%+8px)] w-[340px] max-w-[calc(100vw-2rem)] bg-white border border-[#e8e4dc] rounded-2xl shadow-xl z-50 overflow-hidden">
              ...
              <div v-if="totalNotifs > notifications.length" class="border-t border-[#f7f5f0] px-4 py-2.5">
                <button class="text-[0.75rem] text-[#079272] font-medium hover:underline border-none bg-transparent cursor-pointer w-full text-center" @click="go('notifications'); showNotifs = false">
        ...
        <div class="relative hidden lg:block" data-menu>
          <button class="flex items-center gap-2 px-2 py-1.5 rounded-xl hover:bg-[#f7f5f0] transition-colors border-none bg-transparent cursor-pointer"
            @click.stop="showUserMenu = !showUserMenu; showNotifs = false; showProgramas = false; showSocialInMobile = false">
            <div class="w-8 h-8 rounded-full bg-gradient-to-br from-[#079272] to-[#2464E8] flex items-center justify-center text-white text-xs font-bold select-none flex-shrink-0">
              <div class="w-12 h-8 rounded-2xl overflow-hidden bg-gradient-to-br from-[#079272] to-[#2464E8] flex items-center justify-center">
                <img v-if="currentUser?.profile_picture_url" :src="currentUser.profile_picture_url" :alt="currentUser.full_name" class="w-full h-full object-cover" @error="$event.target.style.display='none'" />
          ...
          <Transition enter-from-class="opacity-0 translate-y-1 scale-95" enter-active-class="transition-all duration-150 origin-top-right" leave-to-class="opacity-0 translate-y-1 scale-95" leave-active-class="transition-all duration-100 origin-top-right">
            <div v-if="showUserMenu" class="absolute right-0 top-[calc(100%+8px)] w-56 bg-white border border-[#e8e4dc] rounded-xl shadow-lg py-1.5 z-50">
              ...
              <button class="w-full flex items-center gap-2.5 px-4 py-2.5 text-[0.8rem] text-[#555] hover:bg-[#f7f5f0] transition-colors cursor-pointer bg-transparent border-none text-left" @click="go('profile'); showUserMenu = false">
        ...
        <div class="flex items-center lg:hidden gap-1">
          <button class="relative w-9 h-9 flex items-center justify-center rounded-xl hover:bg-[#f7f5f0] transition-colors border-none bg-transparent cursor-pointer"
            :class="showSocialInMobile && 'bg-[#f7f5f0]'"
            @click.stop="showSocialInMobile = !showSocialInMobile; closeAll()" data-social title="Redes sociais">
          ...
          <Transition enter-from-class="opacity-0 translate-y-1 scale-95" enter-active-class="transition-all duration-150 origin-top-right" leave-to-class="opacity-0 translate-y-1 scale-95" leave-active-class="transition-all duration-100 origin-top-right">
            <div v-if="showSocialInMobile" class="absolute right-16 top-14 bg-white border border-[#e8e4dc] rounded-xl shadow-lg py-2 px-1 z-50 flex gap-1" data-social>
              <a v-for="social in socialLinks" :key="social.name" :href="social.url" target="_blank" rel="noopener noreferrer"
                ...
                <i :class="social.icon" class="text-xl transition-transform duration-200 group-hover:scale-110" :style="{ color: social.color }"></i>
              </a>
            </div>
          </Transition>
        </div>
    ...
    <Transition enter-from-class="translate-x-full" enter-active-class="transition-transform duration-300 ease-out" leave-to-class="translate-x-full" leave-active-class="transition-transform duration-250 ease-in">
      <aside v-if="isMobileOpen" class="lg:hidden fixed top-0 right-0 h-full w-72 bg-white shadow-2xl z-50 flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto">
          ...
          <div class="px-5 py-4 border-b border-[#f7f5f0]">
            <p class="text-[0.7rem] font-medium text-[#aaa] uppercase tracking-wider mb-3">Siga-nos</p>
            <div class="flex flex-wrap gap-3">
              <a v-for="social in socialLinks" :key="social.name" :href="social.url" target="_blank" rel="noopener noreferrer"
                class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-[#f7f5f0] transition-colors" @click="isMobileOpen = false">
                <i :class="social.icon" class="text-lg" :style="{ color: social.color }"></i>
                <span class="text-[0.8rem] text-[#555]">{{ social.name }}</span>
              </a>
            </div>
          </div>

          <button class="w-full flex items-center justify-between px-5 py-3.5 border-b border-[#f7f5f0] hover:bg-[#f7f5f0] transition-colors border-none bg-transparent cursor-pointer text-left"
            ...
            <span v-if="unreadCount > 0" class="text-[0.65rem] font-bold px-2 py-0.5 bg-red-500 text-white rounded-full">{{ unreadCount }}</span>
          </button>
          ...
          <nav class="px-3 py-3 space-y-0.5">
            ...
            <button @click="go('profile')" class="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-[0.88rem] font-medium text-[#333] hover:bg-[#f7f5f0] hover:text-[#079272] transition-colors border-none bg-transparent cursor-pointer text-left">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>
              </svg>
              Meu perfil
            ...
            <button @click="go('newPost')" class="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-[0.88rem] font-medium text-[#333] hover:bg-[#f7f5f0] hover:text-[#079272] transition-colors border-none bg-transparent cursor-pointer text-left">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
                <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
              </svg>
            ...
            <button v-if="isManager" @click="go('reviewFeed')" class="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-[0.88rem] font-medium text-[#079272] hover:bg-[#f0faf7] transition-colors border-none bg-transparent cursor-pointer text-left">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M9 11l3 3L22 4"/>
                <path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/>
              </svg>
            ...
            <div>
              <button class="w-full flex items-center justify-between gap-3 px-3 py-3 rounded-xl text-[0.88rem] font-medium text-[#555] hover:bg-[#f7f5f0] transition-colors border-none bg-transparent cursor-pointer text-left"
                @click.stop="showProgramas = !showProgramas">
                Programas
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#aaa" stroke-width="2.5" :class="['transition-transform duration-200', showProgramas ? 'rotate-180' : '']">
                  <polyline points="6 9 12 15 18 9"/>
                </svg>
              </button>
              <div v-if="showProgramas" class="pl-6 pb-1 space-y-0.5">
                <button @click="go('embaixadores')" class="w-full text-left px-3 py-2 text-[0.83rem] text-[#777] hover:text-[#079272] transition-colors border-none bg-transparent cursor-pointer rounded-lg hover:bg-[#f7f5f0]">Embaixadores</button>
