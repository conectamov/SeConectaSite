...
function getPerm(userId, field) {
  return permsOverride.value[userId]?.[field] ?? false
}
...
async function togglePerm(user, field) {
  ...
  try {
    ...
    if (status === 403)
      showToast('error', 'Sem permissão para alterar este usuário.')
    else
