...
function validateLogin() {
  ...
  if (!loginForm.value.email.trim())
    loginErr.value.email = 'E-mail obrigatório.'
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(loginForm.value.email))
    loginErr.value.email = 'E-mail inválido.'
  if (!loginForm.value.password)
    loginErr.value.password = 'Senha obrigatória.'
...
function validateRegister() {
  ...
  if (!registerForm.value.full_name.trim())
    registerErr.value.full_name = 'Nome completo obrigatório.'
  if (!registerForm.value.username.trim())
    registerErr.value.username = 'Nome de usuário obrigatório.'
  else if (registerForm.value.username.length > 64)
    registerErr.value.username = 'Máximo 64 caracteres.'
  else if (!/^[a-z0-9_]+$/.test(registerForm.value.username))
    registerErr.value.username = 'Apenas letras minúsculas, números e _.'
  if (!registerForm.value.email.trim())
    registerErr.value.email = 'E-mail obrigatório.'
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(registerForm.value.email))
    registerErr.value.email = 'E-mail inválido.'
  if (!registerForm.value.password)
    registerErr.value.password = 'Senha obrigatória.'
  else if (registerForm.value.password.length < 8)
    registerErr.value.password = 'Mínimo 8 caracteres.'
  if (registerForm.value.password !== registerForm.value.confirmPassword)
    registerErr.value.confirmPassword = 'As senhas não coincidem.'
...
const pwdLabel = computed(() =>
  ['', 'Muito fraca', 'Fraca', 'Razoável', 'Boa', 'Forte'][pwdStrength.value] || ''
)
const pwdColor = computed(() =>
  ['', '#e53e3e', '#f6891a', '#f6c90e', '#079272', '#2464E8'][pwdStrength.value] || ''
)
...
async function handleLogin() {
  ...
  try {
    ...
    errorMsg.value = (status === 401 || status === 400)
...
async function handleRegister() {
  ...
  successMsg.value = ''
  try {
    // POST /api/v1/users/signup → UserRegister { email, username, password, full_name? }
    await register({
    ...
    if (status === 409 || (status === 400 && typeof detail === 'string' && detail.includes('exist')))
      ...
      errorMsg.value = 'Dados inválidos. Verifique os campos.'
    else
  ...
  <div class="min-h-screen bg-[#0c1b32] flex">
    ...
    <div class="hidden lg:flex lg:w-1/2 relative overflow-hidden flex-col justify-between p-12">
      <div
        ...
        aria-hidden="true"
      ></div>
      <div class="absolute w-[350px] h-[350px] rounded-full bg-[#079272] opacity-15 blur-[80px] top-[10%] left-[10%] animate-orb1 pointer-events-none"></div>
      <div class="absolute w-[250px] h-[250px] rounded-full bg-[#2464E8]  opacity-15 blur-[80px] bottom-[20%] right-[5%]  animate-orb2 pointer-events-none"></div>
    ...
    <div class="w-full lg:w-1/2 flex flex-col justify-center items-center px-6 py-12 bg-[#f7f5f0] relative">
      ...
      <!-- Logo mobile -->
      <button
        class="lg:hidden absolute top-6 left-6 text-base font-bold tracking-[-0.02em] text-[#111] bg-none border-none cursor-pointer"
        @click="router.push('/')"
      >conecta<span class="text-[#079272]">.</span></button>
      ...
      <div class="w-full max-w-[400px]">
        ...
        <div class="flex bg-white border border-[#e8e4dc] rounded-xl p-1 mb-8">
          <button
            class="flex-1 text-sm font-semibold py-2.5 rounded-lg transition-all duration-200"
            :class="activeTab === 'login' ? 'bg-[#0d0d0d] text-white shadow-sm' : 'text-[#aaa] hover:text-[#555]'"
            @click="switchTab('login')"
          >Entrar</button>
          <button
            class="flex-1 text-sm font-semibold py-2.5 rounded-lg transition-all duration-200"
            :class="activeTab === 'register' ? 'bg-[#0d0d0d] text-white shadow-sm' : 'text-[#aaa] hover:text-[#555]'"
        ...
        <div v-if="errorMsg" class="flex items-center gap-2.5 bg-red-50 border border-red-200 text-red-600 rounded-xl px-4 py-3 mb-5 text-sm font-medium">
          ...
          {{ errorMsg }}
          <button class="ml-auto text-red-400 hover:text-red-600 leading-none" @click="errorMsg = ''">✕</button>
        </div>
        ...
        <form v-if="activeTab === 'login'" class="flex flex-col gap-4" @submit.prevent="handleLogin">
          ...
          <div>
            <div class="flex justify-between items-baseline mb-1.5">
              ...
              <button type="button" class="text-xs text-[#079272] hover:underline bg-none border-none cursor-pointer font-medium">
                Esqueceu?
              </button>
            </div>
            <div class="relative">
        ...
        <form v-else class="flex flex-col gap-4" @submit.prevent="handleRegister">
          ...
          <div>
            ...
            <input
              ...
              type="text" placeholder="Seu nome" autocomplete="name"
              class="w-full h-11 px-4 bg-white border rounded-xl text-[0.9rem] text-[#111] placeholder:text-[#ccc] focus:outline-none focus:ring-2 focus:ring-[#079272] focus:border-transparent transition-all duration-200"
          ...
          <!-- username -->
          <div>
            <label class="block text-xs font-semibold uppercase tracking-[0.08em] text-[#888] mb-1.5">
              Nome de usuário
            </label>
            <div class="relative">
              ...
              <input
                ...
                type="text" placeholder="seu_username" autocomplete="username"
                maxlength="64"
                ...
                @input="registerForm.username = registerForm.username.toLowerCase().replace(/[^a-z0-9_]/g, ''); delete registerErr.username"
              />
            </div>
            ...
            <div v-if="registerForm.password" class="mt-2">
              <div class="flex gap-1 mb-1">
                <div v-for="i in 5" :key="i" class="h-1 flex-1 rounded-full transition-all duration-300"
                  :style="{ background: i <= pwdStrength ? pwdColor : '#e8e4dc' }"></div>
              </div>
...
.animate-orb2 { animation: orb2 11s ease-in-out infinite reverse; }
</style>
