...
const props = defineProps({
  ...
  repliesMap:     { type: Map,     required: true },
...
const emit = defineEmits([
  ...
  'start-edit',
  'save-edit',
  'cancel-edit',       // FIX: evento próprio para cancelar edição
  'delete-comment',
  'update:edit-text',
  'update:reply-msg',
  'cancel-reply',
...
// FIX: removido useAuth — currentUser já vem por prop, sem import desnecessário
const { getUser, displayName, displayInitial } = useUserCache()

const localReplyingTo = ref(null)
...
const isReplying = computed(() =>
  props.replyingTo === props.comment.id || localReplyingTo.value === props.comment.id
)
const isMyComment = computed(() =>
  props.isAuthenticated && props.comment.author_id === props.currentUser?.id
)
...
const userInitial = computed(() => {
  if (isMyComment.value)
    return (props.currentUser?.full_name || props.currentUser?.username || '?').charAt(0).toUpperCase()
  if (commentAuthor.value) return displayInitial(commentAuthor.value)
  return '?'
})

// FIX: initial do usuário logado para o formulário de reply (diferente do autor do comentário)
...
function handleStartReply() {
  if (props.level < 5) {
    localReplyingTo.value = props.comment.id
    emit('start-reply', props.comment.id)
  }
}

function handleCancelReply() {
  localReplyingTo.value = null
  emit('cancel-reply')
}
...
// FIX: cancelar edição emite evento próprio, não confunde com cancel-reply
...
function handleEditInput(e)  { emit('update:edit-text', e.target.value) }
function handleReplyInput(e) { emit('update:reply-msg', e.target.value) }
</script>

<template>
  <div :class="['comment-item', { 'reply': level > 0 }]">
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-baseline gap-2 mb-1.5 flex-wrap">
          ...
          <span v-if="commentAuthor?.public_title && !isMyComment" class="text-[0.65rem] text-[#aaa]">
            {{ commentAuthor.public_title }}
          </span>
        ...
        <div v-if="isEditing">
          ...
          <div class="flex gap-2 mt-2">
            <button
              class="text-xs font-semibold px-3 py-1.5 bg-[#079272] text-white rounded-lg cursor-pointer hover:bg-[#068060] transition-colors border-none"
              @click="handleSaveEdit"
            >Salvar</button>
            <!-- FIX: chama handleCancelEdit, não handleCancelReply -->
            <button
              class="text-xs font-semibold px-3 py-1.5 bg-[#f7f5f0] text-[#666] rounded-lg cursor-pointer hover:bg-[#e8e4dc] transition-colors border-none"
              @click="handleCancelEdit"
            >Cancelar</button>
          </div>
        </div>

        <p v-else class="text-[0.88rem] font-light text-[#555] leading-relaxed whitespace-pre-wrap break-words">
          {{ comment.message }}
        </p>
        ...
        <div class="flex items-center gap-3 mt-2.5 flex-wrap">
          ...
          <template v-if="isMyComment && !isEditing">
            <button
              class="text-[0.7rem] text-[#bbb] hover:text-[#111] transition-colors cursor-pointer border-none bg-transparent"
              @click="handleStartEdit"
            >Editar</button>
            <button
              class="text-[0.7rem] text-[#bbb] hover:text-red-500 transition-colors cursor-pointer border-none bg-transparent"
              :class="{ 'opacity-40': isDeleting }"
              :disabled="isDeleting"
              @click="handleDelete"
            >{{ isDeleting ? '...' : 'Excluir' }}</button>
          </template>
        ...
        <div v-if="isReplying" class="mt-3 flex gap-2.5">
          <!-- FIX: avatar do usuário logado (currentUserInitial), não do autor do comentário -->
          <div class="w-7 h-7 flex-shrink-0 rounded-full bg-gradient-to-br from-[#079272] to-[#2464E8] flex items-center justify-center text-white text-[0.62rem] font-bold">
            {{ currentUserInitial }}
          </div>
          <div class="flex-1 flex flex-col gap-1.5">
            ...
            <div class="flex gap-2 self-end">
              <button
                class="text-[0.73rem] font-semibold px-4 py-1.5 bg-[#079272] text-white rounded-lg border-none cursor-pointer hover:bg-[#068060] transition-colors disabled:opacity-40 flex items-center gap-1.5"
                :disabled="!replyMsg.trim() || submittingReply"
                @click="handleSubmitReply"
              >
                <svg v-if="submittingReply" class="animate-spin" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <path d="M21 12a9 9 0 1 1-6.219-8.56"/>
                </svg>
                Responder
              </button>
              <button
                class="text-[0.73rem] font-semibold px-4 py-1.5 bg-[#f7f5f0] text-[#666] rounded-lg border-none cursor-pointer hover:bg-[#e8e4dc] transition-colors"
        ...
        <div v-if="replyState?.open" class="mt-4 ml-2 pl-4 border-l-2 border-[#f0ece5] hover:border-[#079272] transition-colors">
          ...
          <template v-else>
            <p v-if="replyState.data.length === 0" class="text-[0.8rem] text-[#999] py-2">
            ...
            <div v-for="reply in replyState.data" :key="reply.id" class="py-3">
              <CommentItem
                ...
                @save-edit="(id, parentId) => $emit('save-edit', id, parentId)"
                @cancel-edit="$emit('cancel-edit')"
                @delete-comment="(id, parentId) => $emit('delete-comment', id, parentId)"
...
@media (max-width: 640px) {
  .comment-item.reply { margin-left: 0.25rem; }
}
</style>
