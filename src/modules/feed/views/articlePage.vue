...
import CommentItem from '@/modules/feed/components/CommentItem.vue'
...
const isMyPost = computed(() =>
  currentUser.value && post.value?.author_id === currentUser.value.id
)
...
const authorDisplayInitial = computed(() => {
  if (isMyPost.value)
    return (currentUser.value?.full_name || currentUser.value?.username || '?').charAt(0).toUpperCase()
  return displayInitial(postAuthor.value)
})

const contentHtml = computed(() =>
  DOMPurify.sanitize(marked.parse(post.value?.content_md || ''))
)
...
async function fetchPost() {
  ...
  try {
    ...
    errorPost.value = e?.response?.status === 404
      ? 'Post não encontrado.'
...
async function fetchComments(reset = false) {
  ...
  try {
    ...
    if (Array.isArray(res.data)) {
      data = res.data; count = res.data.length
    } else if (res.data?.data) {
      data = res.data.data; count = res.data.count ?? data.length
    }
...
// ── Replies ───────────────────────────────────────────
async function fetchReplies(commentId, openAfterLoad = false) {
...
function findCommentById(id) {
  ...
  for (const [, state] of repliesMap.value) {
    found = state.data.find(r => r.id === id)
    if (found) return found
  }
  return null
}
...
async function saveEdit(commentId, parentId = null) {
  if (!editText.value.trim()) return
  try {
    const res = await patch(`/comments/${commentId}`, { message: editText.value.trim() })
    if (parentId) {
      const state = repliesMap.value.get(parentId)
      const reply = state?.data.find(r => r.id === commentId)
...
async function deleteComment(commentId, parentId = null) {
  ...
  try {
    ...
    if (parentId) {
      const state = repliesMap.value.get(parentId)
      if (state) {
        state.data = state.data.filter(r => r.id !== commentId)
        const parent = findCommentById(parentId)
...
// ── Aprovar post — FIX: parâmetro renomeado p/ evitar shadow do ref `post` ───
async function approvePost(targetPost) {
  ...
  try {
    // FIX: era '/post/' (sem 's'), resultava em 404 sempre
    await patch(`/posts/${targetPost.id}`, { approved: true })
    // FIX: atualiza o ref reativo corretamente
    if (post.value) post.value.approved = true

    await Swal.fire({
      icon: 'success', title: 'Post aprovado!',
    ...
    await Swal.fire({
      icon: 'error', title: 'Erro ao aprovar',
  ...
  <div class="min-h-screen bg-[#f7f5f0]">
    ...
    <div v-else-if="errorPost" class="min-h-screen flex flex-col items-center justify-center gap-5 px-8 text-center">
      ...
      <div class="flex gap-3">
        <button class="text-sm font-semibold px-5 py-2 bg-[#0d0d0d] text-white rounded-lg hover:bg-[#079272] transition-colors border-none cursor-pointer"
          @click="fetchPost">Tentar novamente</button>
        <button class="text-sm font-semibold px-5 py-2 bg-white border border-[#e8e4dc] text-[#555] rounded-lg hover:border-[#0d0d0d] transition-colors border cursor-pointer"
          @click="router.push({ name: 'feed' })">Voltar ao feed</button>
      </div>
    </div>
    ...
    <template v-else-if="post">
      ...
      <header class="relative overflow-hidden bg-[#0c1b32] pt-20 px-8 pb-0">
        <div class="relative z-10 max-w-[1000px] mx-auto">
          ...
          <!-- FIX: removido '>' solto antes do texto -->
          ...
          <div class="flex items-center justify-between bg-white/5 border border-white/[0.08] rounded-t-xl p-4 md:p-5">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 rounded-full bg-gradient-to-br from-[#079272] to-[#2464E8] flex items-center justify-center text-white text-sm font-bold">
                {{ authorDisplayInitial }}
              </div>
              <div>
                <div class="text-[0.88rem] font-semibold text-white">{{ authorDisplayName }}</div>
                <div v-if="postAuthor?.username || (isMyPost && currentUser?.username)"
      ...
      <div v-if="post.cover_url" class="max-w-[1000px] mx-auto">
        <img :src="post.cover_url" :alt="post.title"
          class="w-full h-[380px] max-md:h-52 object-cover block"/>
      </div>
      ...
      <div class="max-w-[1000px] mx-auto px-6 md:px-8 pb-16 grid grid-cols-1 md:grid-cols-[1fr_180px] gap-12 items-start">
        ...
        <aside class="pt-12 sticky top-20 hidden md:flex flex-col gap-4">
          ...
          <div class="bg-white border border-[#e8e4dc] rounded-xl p-5">
            ...
            <div v-for="s in [{ l: 'Curtidas', v: likeCount }, { l: 'Comentários', v: commentsCount }, { l: 'Leitura', v: readTime }]"
              :key="s.l"
      ...
      <div class="max-w-[760px] mx-auto px-6 md:px-8 pb-28">
        ...
        <div v-if="isAuthenticated" class="flex gap-3 mb-10">
          <div class="w-9 h-9 flex-shrink-0 rounded-full bg-gradient-to-br from-[#079272] to-[#2464E8] flex items-center justify-center text-white text-xs font-bold">
            {{ currentUserInitial }}
          </div>
          <div class="flex-1 flex flex-col gap-2">
            ...
            <button
              ...
              @click="submitComment"
            >
              <svg v-if="submitting" class="animate-spin" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div v-else class="mb-8 p-4 bg-[#f7f5f0] border border-[#e8e4dc] rounded-xl text-sm text-center text-[#888]">
          <button class="text-[#079272] font-semibold hover:underline border-none bg-transparent cursor-pointer"
            @click="router.push({ name: 'login' })">Faça login</button> para comentar.
        </div>
        ...
        <div v-else-if="!loadingComments && comments.length === 0"
          class="py-12 text-center text-sm text-[#aaa]">
        ...
        <div v-else class="flex flex-col divide-y divide-[#f0ece5]">
          <div v-for="comment in comments" :key="comment.id" class="py-5">
            <CommentItem
              ...
              @start-reply="(id) => { replyingTo = id; replyMsg = '' }"
        ...
        <div v-if="commentHasMore && !loadingComments" class="flex justify-center pt-6">
          <button
            ...
            @click="loadMoreComments">Ver mais comentários</button>
        </div>
