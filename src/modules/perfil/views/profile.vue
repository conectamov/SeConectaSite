...
// 'info' | 'notifications' | 'security' | 'activity'
...
async function saveInfo() {
  ...
  try {
    ...
    infoError.value = e?.response?.status === 409
      ? 'Email já em uso.'
...
const pwStrengthLabel = computed(() =>
  ['', 'Muito fraca', 'Fraca', 'Média', 'Forte', 'Muito forte'][pwStrength.value] || ''
)
const pwStrengthColor = computed(() =>
  ['', '#e53e3e', '#e53e3e', '#d97706', '#079272', '#079272'][pwStrength.value] || ''
)
...
async function deleteAccount() {
  ...
  try {
    ...
    deleteError.value = 'Erro ao deletar conta. Tente novamente.'
    deleting.value    = false
  }
}

// --------------------------------------------------------------
// TAB: ATIVIDADE — posts (inclui não aprovados)
// --------------------------------------------------------------
const myPosts = computed(() => currentUser.value?.posts ?? [])
const pendingPosts   = computed(() => myPosts.value.filter(p => !p.approved))
const publishedPosts = computed(() => myPosts.value.filter(p => p.approved))
            ...
            <div class="bg-white border border-[#e8e4dc] rounded-2xl p-6">
              ...
              <div v-else class="flex flex-col divide-y divide-[#f7f5f0]">
                <div
                  ...
                  @click="post.approved && post.slug && router.push({ name: 'article', params: { slug: post.slug } })"
                >
